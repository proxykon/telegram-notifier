...
final class TelegramTransport extends AbstractTransport
{
    ...
    protected function doSend(MessageInterface $message): SentMessage
    {
        ...
        if (isset($options['upload'])) {
            foreach ($options['upload'] as $option => $document) {
                $options[$option] = fopen($document['path'], 'r');
                $fNamePart = explode('.', $document['fileName']);
                stream_context_set_option($options[$option], 'http', 'filename', $document['fileName']);
                stream_context_set_option($options[$option], 'http', 'content_type', $document['mimeType']);
            }
